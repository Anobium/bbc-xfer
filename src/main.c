/* $Id: main.c,v 1.7 2005/01/16 10:35:10  jon Exp $
 *
 * XFer in C
 *
...
int main(int argc, char *argv[])
{
  ...
  default_configuration(&config, home) ;
  ...
  printf("Connected at %ld baud, handshake %s, timeout %.2f seconds,\n"
         ...
         baudtoi(config.baudrate),
         (config.handshake == HANDSHAKE_DSR_DTR) ? "DSR/DTR"
         : ((config.handshake == HANDSHAKE_CTS_RTS) ? "CTS/RTS"
         : "XON/XOFF"),
         config.timeoutdelay / 100.0,
...
/*
 * $Log: main.c,v $
 * Revision 1.8  2005/01/22 08:30:13  jon
 * Added support for transferring disc images back from PC to BBC
 *
 * Revision 1.7  2005/01/16 10:35:10  jon
 * Added support for ADFS disc image
 *
